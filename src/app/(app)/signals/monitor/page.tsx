...
export default function SignalMonitorPage() {
  ...
  const exportPayload = useMemo((): ExportPayload => {
    ...
    return {
      title: "STRATEGOS — Signal Monitor Report",
      generated: new Date().toUTCString(),
      stats: [
        { label: "Total Signals",   value: rawSignals.length },
        { label: "Alert Signals",   value: alerts },
        { label: "Active Layers",   value: layers.length },
        { label: "Positive",        value: sentimentData.find((s) => s.label === "Positive")?.pct ?? 0 + "%" },
        { label: "Alert %",         value: sentimentData.find((s) => s.label === "Alert")?.pct ?? 0 + "%" },
      ],
      tables: [
        {
          title: "Signal Feed",
          headers: ["Layer", "Source", "Content", "Score", "Confidence %", "Alert", "Severity", "Time"],
          rows: filtered.map((s) => [
            s.mapped.layer,
            s.mapped.source,
            s.mapped.content?.slice(0, 120) ?? "",
            s.mapped.normalizedScore != null ? s.mapped.normalizedScore.toFixed(2) : "",
            s.relevance,
            s.mapped.alertFlag ? "YES" : "no",
            s.mapped.alertSeverity ?? "",
            s.mapped.timeAgo,
          ]),
        },
        {
          title: "Layer Distribution",
          headers: ["Layer", "% of Signals"],
      ...
      notes: "Generated by STRATEGOS AI Intelligence Platform.",
    };
